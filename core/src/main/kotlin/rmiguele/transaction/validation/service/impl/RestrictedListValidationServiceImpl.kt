...
class RestrictedListValidationServiceImpl(private val personRepository: PersonRepository, private val violationService: ViolationService) : RestrictedListValidationService {
    ...
    override fun validate(command: RestrictedListValidationCommand) {
        personRepository.findOne(command.senderCode)?.also {
            if (it.situation == PersonSituation.ILLEGAL) {
                violationService.addViolation(AddViolationCommand(command.transactionCode, "Remetente \"${it.code}\" em situação ilegal"))
            }
        }

        personRepository.findOne(command.receiverCode)?.also {
            if (it.situation == PersonSituation.ILLEGAL) {
                violationService.addViolation(AddViolationCommand(command.transactionCode, "Receptor \"${it.code}\" em situação ilegal"))
            }
        }
    }
